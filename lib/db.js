...
export async function dbSaveOrders(orders, batchId) {
  ...
  for (const order of orders) {
    await sql`
      INSERT INTO orders (
        batch_id, ref_code, sender_name, sender_phone, sender_address,
        receiver_name, receiver_phone, receiver_address,
        weight, item_quantity, temp_zone, remark, status, created_at
      ) VALUES (
        ${batchId},
        ${order.refCode || null},
...
export async function dbGetOrdersPaginated(page = 1, pageSize = 20, filters = {}) {
  ...
  // neon() supports sqlRaw.query() for parameterized raw SQL
  const countResult = await sqlRaw.query(
    `SELECT COUNT(*) as total FROM orders ${where}`,
    params
  );
  const total = parseInt(countResult.rows[0].total, 10);

  const dataResult = await sqlRaw.query(
