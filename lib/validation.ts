...
import type { Order, ValidationError, ValidationSummary } from './types';

/**
 * Validate all rows at once — returns ALL errors grouped by row
 * Each error: { row, field, fieldLabel, message, level: 'error'|'warning' }
 */
export function validateAllRows(rows: Order[]): ValidationError[] {
  const errors: ValidationError[] = [];
  const refCodeMap = new Map<string, number>(); // track duplicates within batch
  ...
  rows.forEach((row, index) => {
    ...
    SYSTEM_FIELDS.forEach(field => {
      if (field.required) {
        const val = String((row as Record<string, unknown>)[field.key] || '').trim();
        if (!val) {
          errors.push({
            row: rowNum,
            dataIndex: index,
    ...
    (['senderPhone', 'receiverPhone'] as const).forEach(phoneField => {
      const val = String((row as Record<string, unknown>)[phoneField] || '').trim();
      if (val && !/^1[3-9]\d{9}$/.test(val) && !/^0\d{2,3}-?\d{7,8}$/.test(val)) {
        const label = SYSTEM_FIELDS.find(f => f.key === phoneField)?.label ?? phoneField;
        errors.push({
    ...
    (['senderAddress', 'receiverAddress'] as const).forEach(addrField => {
      const val = String((row as Record<string, unknown>)[addrField] || '').trim();
      if (val && val.length < 8) {
        const label = SYSTEM_FIELDS.find(f => f.key === addrField)?.label ?? addrField;
        errors.push({
 ...
 */
export function checkDuplicatesAgainstDB(rows: Order[], existingRefCodes: string[]): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
export function getValidationSummary(errors: ValidationError[]): ValidationSummary {
 ...
 */
export function getCellErrors(errors: ValidationError[], dataIndex: number, fieldKey: string): ValidationError[] {
